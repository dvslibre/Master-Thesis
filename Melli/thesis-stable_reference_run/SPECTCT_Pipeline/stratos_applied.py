...
Überträgt die Stratos-Idee auf klinische SPECT/CT-Daten:
...
python stratos_applied.py \
  ...
  --ap            /home/mnguest12/projects/thesis/SPECTCT_Pipeline/data/example_01/results/scintigraphy_sim/projection_AP.npy \
  --pa            /home/mnguest12/projects/thesis/SPECTCT_Pipeline/data/example_01/results/scintigraphy_sim/projection_PA.npy \
  --output_dir    /home/mnguest12/projects/thesis/SPECTCT_Pipeline/data/example_01/results/stratos_applied \
    --scatter_sigma_xy 1.0 \
  --coll_sigma_xy 1.0 \
...
def create_body_mask_from_ct(ct_resampled: sitk.Image, hu_threshold: float = -400.0) -> sitk.Image:
    """Erzeugt eine grobe Body-Maske (alles außer Luft) aus CT im SPECT-Space.
...
def build_group_mask_from_substrings(
    ...
    resampelt sie in den SPECT-Space und bildet die logische Union.

    Rückgabe:
      bool-Array (z,y,x) oder None, falls nichts gefunden / zu wenig Voxel.
    """
    spect_shape = sitk.GetArrayFromImage(spect_img).shape
    union = np.zeros(spect_shape, dtype=bool)
    count_files = 0
    total_vox = 0
    ...
    for fname in all_files:
        ...
        mask_spect = resample_mask_to_spect(mask_ct, spect_img)
        arr = sitk.GetArrayFromImage(mask_spect) > 0

        if arr.shape != spect_shape:
...
def load_organ_masks_with_others(
    ...
    """Lädt explizite Organmasken (ROI_FILES), resampelt sie in SPECT-Space,
    ...
    """
    spect_shape = sitk.GetArrayFromImage(spect_img).shape  # (z,y,x)
    ...
    for organ, fname in roi_files.items():
        ...
        if arr.shape != spect_shape:
            raise RuntimeError(
                f"Maskenshape passt nicht zum SPECT: {organ}, mask shape={arr.shape}, spect shape={spect_shape}"
            )
...
def build_system_matrix(
    ...
    - jede Spalte entspricht einer 3D-Organmaske (im SPECT-Space),
    ...
    """
    spacing_x, spacing_y, spacing_z = spect_img.GetSpacing()
    ...
    for organ, mask_arr_zyx in organ_masks_zyx.items():
        ...
        if not mask_bool.any():
            print(f"[WARNUNG] Maske für {organ} ist im SPECT-Space leer.")
            continue
...
def run_stratos_applied(
    ...
    # --- SPECT/CT einlesen und µ-Map erzeugen ---
    print(f"[INFO] Lese SPECT-DICOM aus: {spect_dicom_dir}")
    spect_img = scinti.read_dicom_series(spect_dicom_dir)
    if spect_img.GetDimension() == 4:
        print("[INFO] SPECT ist 4D – reduziere auf 3D (sum).")
        spect_img = scinti.collapse_4d_to_3d(spect_img, mode="sum")
    spect_img = scinti.fix_direction_if_degenerate(spect_img, name="SPECT")
    ...
    ct_img = scinti.reorient_to_lps(ct_img)
    ct_img = scinti.fix_direction_if_degenerate(ct_img, name="CT")
...
def main():
    parser = argparse.ArgumentParser(
        description="Stratos-Ansatz auf klinische SPECT/CT-Daten (Organbasierte Szinti-Faktorisierung mit 'others')."
    )
    parser.add_argument("--spect_dicom_dir", required=True,
                        help="DICOM-Ordner der SPECT-Serie (Aktivitätsvolumen).")
