...
Dieses Repository enthält eine Reihe von Skripten, die zusammen eine Auswertungs-Pipeline für klinische SPECT/CT-Daten bilden.  
...
2. **(V)OIs definieren & auswerten**
   → `VOI_evaluation.py`
   Berechnung von Kenngrößen (z. B. Voxel, Counts mean / min / max) für 3D-VOIs. Dafür werden die Segmente in das SPECT gelegt.

3. **Simulierte Szintigrafie aus PET/CT**  
   → `scintigraphy_simulation.py`  
   Aus 3D-SPECT/CT-Daten werden planare Projektionen (AP/PA) berechnet, die annähernd einer Gamma-Kamera-Szintigrafie entsprechen. Nutzung des Gammakamera-Modells vom stratos-Ansatz.
