...
python VOI_evaluation.py \
    ...
    --output_csv    /home/mnguest12/projects/thesis/PETCT_Pipeline/data/example_01/results/PET/pet_stats.tsv
"""
...
def resample_image_to_target(
    ...
    """
    Allgemeines Resampling eines Bildes (z.B. CT) in den Raum eines Referenzbildes (z.B. PET).
    """
    resampler = sitk.ResampleImageFilter()
...
def compute_statistics(pet_img: sitk.Image, mask_img: sitk.Image):
    """
    Berechnet Summe, Mittelwert, Min, Max für alle PET-Werte innerhalb der Maske.
    """
    pet_arr = sitk.GetArrayFromImage(pet_img)      # shape: [z, y, x]
    mask_arr = sitk.GetArrayFromImage(mask_img)

    # Maske: alles > 0 als 'innerhalb'
    mask_bool = mask_arr > 0

    # PET-Werte innerhalb der VOI
    values = pet_arr[mask_bool]
...
def create_organ_visualizations(
    organ: str,
    pet_img: sitk.Image,
    mask_img_pet_space: sitk.Image,
    out_dir: str,
    ct_img_pet_space: sitk.Image | None = None,
):
    """
    Erzeugt für ein Organ:
      - Overlay-Bild (CT grau + PET Heatmap + Maskenkontur)
      - Histogramm der PET-Counts im Organ
    und speichert beides in einer gemeinsamen Figure als PNG.
    """
    os.makedirs(out_dir, exist_ok=True)

    pet_arr = sitk.GetArrayFromImage(pet_img)
    mask_arr = sitk.GetArrayFromImage(mask_img_pet_space) > 0

    if not mask_arr.any():
        # Nichts in der Maske -> keine Visualisierung nötig
        return

    if ct_img_pet_space is not None:
        ct_arr = sitk.GetArrayFromImage(ct_img_pet_space)
    else:
        ct_arr = None
    ...
    if ct_slice is not None:
        ...
        ax0.imshow(pet_slice, cmap="gray")

    ax0.imshow(pet_slice, cmap="hot", alpha=0.5)
...
def run_evaluation(
    ...
    # Serienlabel aus dem DICOM-Verzeichnis (z.B. PET_WB_TRUEX2I14S0MM_AC_0102)
    series_label = os.path.basename(os.path.normpath(pet_dicom_dir))

    print(f"Lese PET-DICOM-Serie aus: {pet_dicom_dir}")
    pet_img = read_dicom_series(pet_dicom_dir)

    # CT optional einlesen und in PET-Space bringen
    ct_img_pet_space = None
    if ct_dicom_dir is not None:
        print(f"Lese CT-DICOM-Serie aus: {ct_dicom_dir}")
        ct_img = read_dicom_series(ct_dicom_dir)
        print("Resample CT -> PET-Space ...")
        ct_img_pet_space = resample_image_to_target(
            ct_img, pet_img, interpolator=sitk.sitkLinear, default_value=-1024
        )
    ...
    for organ, filename in VOI_FILES.items():
        ...
        # Maske in PET-Space bringen
        mask_resampled = resample_mask_to_pet(mask_img_ct_space, pet_img)

        stats = compute_statistics(pet_img, mask_resampled)
        stats["organ"] = organ
        ...
        if fig_dir is not None:
            create_organ_visualizations(
                organ=organ,
                pet_img=pet_img,
                mask_img_pet_space=mask_resampled,
                out_dir=fig_dir,
                ct_img_pet_space=ct_img_pet_space,
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Auswertung von PET-Werten innerhalb von VOI-Masken"
    )
    parser.add_argument(
        "--pet_dicom_dir",
        required=True,
        help="Pfad zum DICOM-Ordner der PET-Serie",
    )
    parser.add_argument(
        "--mask_dir",
        required=True,
