...
echo "🚀 Starting act-loss-weight sweep on $HOSTNAME"
...
# Uses job-id namespacing (like your baseline), then per TAG folder
BASE_DIR="${ROOT}/results_spect_test_1/actW_sweep_run_${SLURM_JOB_ID}"
mkdir -p "${BASE_DIR}"
...
for W in "${ACT_WEIGHTS[@]}"; do
  ...
  echo ""
  echo "============================================================"
  echo "🔁 Starting sweep run: ${TAG}  (act-loss-weight=${W})"
