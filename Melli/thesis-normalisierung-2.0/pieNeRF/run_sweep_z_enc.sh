...
#SBATCH --output=/home/mnguest12/slurm/sweep_zEnc.%j.out
#SBATCH --error=/home/mnguest12/slurm/sweep_zEnc.%j.err
...
echo "🚀 Starting z-enc-alpha sweep on $HOSTNAME"
echo "📅 Job started at: $(date)"
echo "🧠 GPUs assigned: ${SLURM_JOB_GPUS:-<unset>}"
echo "CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES:-<unset>}"

# 1) Conda env (match baseline) — keep exactly as you had it
...
# IMPORTANT: actW fixed to baseline
ACT_LOSS_WEIGHT="3.0"
...
# z-enc sweep variable: Z_ENC_ALPHA (set per run)
...
BASE_DIR="${ROOT}/results_spect_test_1/zEnc_sweep_run_${SLURM_JOB_ID}"
mkdir -p "${BASE_DIR}"
echo "📁 BASE_DIR=${BASE_DIR}"

# --- robust cleanup for results_spect symlink (same style as baseline) ---
RESTORE_DIR=""
cleanup_link() {
...
for A in "${Z_ENC_ALPHAS[@]}"; do
  ...
  echo ""
  echo "============================================================"
  echo "🔁 Starting sweep run: ${TAG}  (z-enc-alpha=${A})"
  ...
  # Record basic metadata (baseline-like)
  {
    ...
    echo "tag: ${TAG}"
    echo "z_enc_alpha: ${A}"
    echo "act_loss_weight_fixed: ${ACT_LOSS_WEIGHT}"
    echo "cuda_visible_devices: ${CUDA_VISIBLE_DEVICES:-<unset>}"
  ...
  if [ $exit_code -ne 0 ]; then
    ...
    # keep outputs for debugging; continue to next A
  fi
